# -*- coding: utf-8 -*-
"""
Created on Thu May  2 10:20:12 2019
...
g = user_102[['customer_id','Day','Month','Year']].groupby(['Day','Month','Year']).agg('count')

#assign customerid to custvisits
g=pd.DataFrame(g)
g.columns=['custvisits']
...
#labels
labels=g.loc[:,['custvisits']].values
...
#features
featuresx=g.loc[:,['Month','Year']].values
featuresx=pd.DataFrame(featuresx, columns=["month","year"])

#labels
labelsy=g.loc[:,['visits']].values
...
# Splitting the dataset into the Training set and Test set
from sklearn.model_selection import train_test_split
features_train,features_test,labels_train,labels_test=train_test_split(featuresx,labelsy,test_size=0.05, random_state=0)
...
with open('modelvmonths.pkl', 'rb') as handle:
    reg = pickle.load(handle)   

p_features3=np.array([[1,2019],[2,2019],[3,2019],[4,2019],[5,2019]])
p_result3=reg.predict(p_features3)
